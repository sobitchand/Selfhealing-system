...
class UIHeuristicEngine:
    ...
    def _extract_broken_value(self, broken_selector):
        ...
        For XPath expressions, extracts the meaningful identifier inside
        (@id='...', @class='...', text()='...', normalize-space()='...').
        Strips leading css punctuation so we compare on the bare identifier.
        """
        selector = str(broken_selector)

        # XPath: try to pull out @id, @class, text(), normalize-space() values
        # before falling back to the whole expression.
        if "xpath" in selector.lower() or selector.startswith("//"):
            for pattern in [
                r"@id\s*=\s*['\"]([^'\"]*)['\"]",
                r"@class\s*=\s*['\"]([^'\"]*)['\"]",
                r"@name\s*=\s*['\"]([^'\"]*)['\"]",
                r"normalize-space\s*\(\s*\)\s*=\s*['\"]([^'\"]*)['\"]",
                r"text\s*\(\s*\)\s*=\s*['\"]([^'\"]*)['\"]",
        ...
        raw = m.group(1) if m else selector
    ...
    def select_target_fingerprint(self, broken_selector, min_ratio=40.0):
        ...
        for key, golden in self.fingerprints.items():
            # Try element_id first, then locator_value, then key, then inner_text
            # (so text-based XPath locators like //button[normalize-space()='Cancel']
            #  resolve to the fingerprint whose visible text was "Cancel")
            candidates = [
                golden.get("element_id") or "",
                str(golden.get("locator_value") or ""),
                key,
                str(golden.get("inner_text") or ""),
            ]
...
class DynamicInfrastructureHealer:
    ...
    def analyze_and_heal_system(self):
        ...
        if is_down:
            self.execute_infrastructure_heal(current_state, disk_stress=False, error_stress=False)
        elif disk_stress and error_rate_stress:
            self.execute_infrastructure_heal(current_state, disk_stress=True, error_stress=True)
        elif disk_stress:
            self.execute_infrastructure_heal(current_state, disk_stress=True, error_stress=False)
